define("bus/main06-debug", ["module-tools/sftools-debug"], function(require, exports, modules) {
    function update() {
        for (var i = 0; i < sf.doubleBall["double"].length; i++) {
            var node = document.createElement("li"),
                textnode = document.createTextNode(sf.doubleBall["double"][i]);
            node.appendChild(textnode), document.getElementsByTagName("ul")[0].appendChild(node)
        }
        document.getElementsByTagName("ul")[0].lastChild.style.backgroundColor = " #43a5eb"
    }
    var sf = require("module-tools/sftools-debug");
    sf.doubleBall.start(), update(), document.getElementsByTagName("button")[0].onclick = function() {
        document.getElementsByTagName("ul")[0].innerHTML = "", sf.doubleBall.start(), update()
    }
});
"use strict";
