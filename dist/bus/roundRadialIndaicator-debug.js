define("bus/roundRadialIndaicator-debug", ["js/jQuery-debug", "js/jquery-debug.knob", "jquery-debug"], function(require, exports, module) {
    function animate(me, begin, end, time, fn, type) {
        var beginTime = (new Date).getTime(),
            diffPos = end - begin;
        cancelAnimationFrame(me.animateId), me.animateId = requestAnimationFrame(function loop() {
            var changeTime = (new Date).getTime() - beginTime,
                diff = Math.floor(diffPos * easing[type ? "outEase" : "inEase"](changeTime / time)) + begin;
            changeTime < time ? (fn(diff), me.animateId = requestAnimationFrame(loop)) : fn(end)
        })
    }
    var $ = require("js/jQuery-debug");
    require("js/jquery-debug.knob");
    var WIN = ($(".dial").knob({
            width: 100,
            height: 100,
            thickness: .1,
    ...
    animate($(".dial"), 0, 100, 15e3, function(val) {
        $(".dial").val(val).trigger("change");
        var red = Math.floor(2 * val),
            green = 200 - Math.floor(2 * val);
...
! function(factory) {
    "object" == typeof exports ? module.exports = factory(require("jquery-debug")) : "function" == typeof define && define.amd ? define("js/jquery-debug.knob", ["jquery-debug"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var k = {},
        max = Math.max,
